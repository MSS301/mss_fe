...
export default function Wallet({ token, userId }: Props) {
  ...
  const PAGE_SIZE = 10; // số giao dịch hiển thị mỗi trang (FE)
  ...
  async function loadWalletData() {
    try {
      setLoading(true);
      setError(null);
      const walletData = await getMyWallet(token, userId);
      ...
      while (true) {
        ...
        if (res && res.content) {
          allTxs.push(...res.content);
        }
        if (!res || res.last) break;
        bePage += 1;
      }
  ...
  // Recompute totalPages when transactions or activeTab changes
  useEffect(() => {
    const filtered = transactions.filter((t) => {
      if (activeTab === "topup") return t.transaction_type === "TOP_UP";
      return (
      ...
      <div className="wallet-stats">
        ...
        <div className="stat-item">
          ...
          <span className="stat-value expense">
            {(() => {
              const walletCount = transactions.filter(
                (t) =>
                  t.transaction_type === "TOKEN_DEDUCTION" ||
                  t.transaction_type === "TOKEN_ADDITION"
              ).length;
              ...
              return (
                <div className="transactions-empty">
                  <div className="transactions-empty-icon">
                    <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
                      <path
                        d="M16 16H48C50.2091 16 52 17.7909 52 20V44C52 46.2091 50.2091 48 48 48H16C13.7909 48 12 46.2091 12 44V20C12 17.7909 13.7909 16 16 16Z"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        ...
                        <div className="transaction-left">
                          ...
                          >
                            {isIncome ? (
                              ...
                              >
                                <path
                                  d="M12 19V5M12 19L18 13M12 19L6 13"
                                  stroke="currentColor"
                                  strokeWidth="2.5"
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
